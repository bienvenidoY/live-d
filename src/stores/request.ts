...
import { Client, ServiceClient } from '@/lib/request'
import {secretStorageAtom} from "@/stores/jotai";
...
export function useServiceAPIInfo() {
    const [defaultSecret] = useAtom(secretStorageAtom)
    ...
    return {
        hostname: 'nht.1024tool.vip',
        secret: qs.get('secret') ?? defaultSecret ?? '' ,
        protocol: 'https'
    }
}
...
export function useServiceClient () {
    ...
    return new ServiceClient(`${protocol}://${hostname}/`)
}
